...
function _mdl(segment::Vector{Float32}, BP::Vector{UInt32})::Float32
    N = length(segment)
    BPi = unique!(sort!(vcat(UInt32[1], BP, UInt32[N])))
    p = length(BPi) - 1
    RSS = 0.0
    CL = 0.0
    @inbounds for k in 2:length(BPi)
        start_idx = BPi[k-1]
        end_idx = BPi[k] - 1
        if end_idx < start_idx
            continue
        end
        Nseg = end_idx - start_idx + 1
    ...
    return Float32(p*log(N) + 0.5*CL + (N/2)*log(RSS/N))
end
...
function detect_single_breakpoint(data::Vector{Float32}, min_seg::UInt16=UInt16(300))::Vector{UInt32}
    ...
    min_seg_minus_1 = min_seg - 1
    mean1 = mean(view(data, 1:min_seg_minus_1))
    mean2 = mean(view(data, min_seg:n))
    
    logL1 = 0.0f0
    logL2 = 0.0f0
    @inbounds for i in 1:min_seg_minus_1
        diff = data[i] - mean1
        logL1 += diff * diff
    end
    @inbounds for i in min_seg:n
        diff = data[i] - mean2
        logL2 += diff * diff
    end
    ...
    for i in min_seg + 1:n - min_seg - 1
        ...
        data_i_minus_mean1 = data[i] - mean1
        data_i_minus_new_mean2 = data[i] - new_mean2
        
        newL1 = logL1 + data_i_minus_mean1 * data_i_minus_mean1 - i*(diff1*diff1)
        newL2 = logL2 - data_i_minus_new_mean2 * data_i_minus_new_mean2 + (n - i + 1)*(diff2*diff2)
        Nloglik = newL1 + newL2
...
function detect_double_breakpoint(data::Vector{Float32}, min_seg::UInt16=UInt16(300)) ::Vector{UInt32}
    ...
    for i in min_seg:n - 2 * min_seg - 1
        ...
        mu1 = cumx_i / i_plus_1
        mu1_sq = mu1 * mu1
        logL1 = cumz_i - 2*mu1*cumx_i + i_plus_1*mu1_sq
        ...
        for j in i + min_seg:n - min_seg - 1
            ...
            cumx_diff3 = cumx_end - cumx_j
            mu3 = cumx_diff3 / l3
            mu3_sq = mu3 * mu3
            logL3 = cumz_end - cumz_j - 2*mu3*cumx_diff3 + l3*mu3_sq
...
function stepstat_mdl(data::Vector{Float32}, BP::Vector{UInt32}, threshold::Float32) :: Tuple{Vector{UInt32}, Vector{Float32}}
    ...
    stepvalue = Vector{Float32}(undef, n_segs)
    
    # Calculate segment means
    prev_idx = 1
    @inbounds for i in 1:n_breaks
        b_idx = BP[i]
        sum_val = 0.0f0
        for j in prev_idx:b_idx
            sum_val += data[j]
        end
        stepvalue[i] = sum_val / (b_idx - prev_idx + 1)
        prev_idx = b_idx + 1
    end
    ...
    @inbounds begin
        ...
        stepvalue[n_segs] = sum_val / (n - prev_idx + 1)
    end
    
    # Filter by jumps
    filtered = Vector{UInt32}()
    sizehint!(filtered, n_breaks)
    
    @inbounds for i in 1:n_breaks
        if abs(stepvalue[i+1] - stepvalue[i]) > threshold
            push!(filtered, BP[i])
        end
    end
