...
# ╔═╡ ec78ee0c-0315-4e87-a717-e8113c155b70
begin
	import Pkg
	...
	Pkg.add(["PyCall", "PlutoUI", "Plots", "OpenSSL_jll", "HypothesisTests"])
	Pkg.build("PyCall")
	using PyCall
...
# ╔═╡ 0ec2bb0c-5695-4e6a-b327-32c11ee590f7
fill([1], 3)

# ╔═╡ 85c6ec64-6369-413b-8a2d-db71ac7e9aaf
what_first_dict
...
function plot_idealization_for_methods(data::Dict{String, Vector{Float64}}, method_outputs::Vector{MethodOutput}, T_left::Float64, T_right::Float64, Δt::Float64)
...
begin
	...
	for method in methods
		...
		if typeof(method_output) <: MikaMethodOutput
			vals = sort(unique(method_output.idealized_data))
			mapped = (method_output.idealized_data .== vals[2])
			approx_idealization = Vector{Int8}(mapped)
		else
